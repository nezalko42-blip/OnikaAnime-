// ============================================
// API МОДУЛЬ ONIKAANIME — ГИБРИД (Anilibria + Shikimori)
// ============================================
...
const API = {
    ...
    _shikimoriCacheTTL: 60 * 60 * 1000,

    // ============================================
    // БАЗОВЫЕ GET/POST ЗАПРОСЫ (Anilibria)
    // ============================================
    ...
    // ============================================
    // SHIKIMORI — FALLBACK ДЛЯ РУССКИХ НАЗВАНИЙ
    // ============================================
    async getShikimoriTitle(query, year = null) {
        if (!query || query.length < 2) return null;
        
        const cacheKey = 'shiki|' + query.toLowerCase().trim() + '|' + (year || '');
        ...
        try {
            ...
            if (data.errors) {
                console.warn('⚠️ Shikimori errors:', data.errors[0]?.message);
                return null;
            }
    ...
    async _getLatestReleases(limit = 48) {
        ...
        if (!data || !Array.isArray(data) || data.length === 0) {
            ...
            if (fallbackData && fallbackData.data) data = fallbackData.data;
        }
...
window.API = API;
console.log('✅ API модуль (гибрид Anilibria + Shikimori) загружен');
