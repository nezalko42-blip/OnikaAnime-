...
function getGenreColor(genre) {
    const colorMap = {
        'Экшен': '#ff2d78', 'Приключения': '#f39c12', 'Комедия': '#f1c40f',
        'Драма': '#8e44ad', 'Фэнтези': '#3498db', 'Романтика': '#e84393',
        'Фантастика': '#00b894', 'Повседневность': '#636e72', 'Триллер': '#c0392b',
        'Ужасы': '#6c5ce7', 'Спорт': '#e67e22', 'Музыка': '#00cec9',
        'Меха': '#4d8aff', 'Сёдзё': '#fd79a8', 'Сёнэн': '#0096ff',
        'Игра': '#a29bfe', 'Мистика': '#b44aff', 'Детектив': '#00f5ff'
    };
...
function updateUI() {
    ...
    if (user) {
        ...
        const commentCount = (window._myCommentsCache && window._myCommentsCache.user === user.name)
            ? window._myCommentsCache.count : 0;
        ...
        nav.innerHTML = `
            <a data-page="home" onclick="navigate('home'); closeMenu();"><span class="icon">🏠</span><span>Главная</span></a>
            <a data-page="favorites" onclick="navigate('favorites'); closeMenu();"><span class="icon">❤️</span><span>Избранное</span>${favCount > 0 ? `<span class="sidebar-badge">${favCount > 99 ? '99+' : favCount}</span>` : ''}</a>
            <a data-page="mycomments" onclick="navigate('mycomments'); closeMenu();"><span class="icon">💬</span><span>Мои комментарии</span>${commentCount > 0 ? `<span class="sidebar-badge">${commentCount > 99 ? '99+' : commentCount}</span>` : ''}</a>
            <a data-page="achievements" onclick="navigate('achievements'); closeMenu();"><span class="icon">🏆</span><span>Достижения</span></a>
            <a data-page="profile" onclick="navigate('profile'); closeMenu();"><span class="icon">👤</span><span>Профиль</span></a>
            <a data-page="settings" onclick="navigate('settings'); closeMenu();"><span class="icon">⚙️</span><span>Настройки</span></a>
        `;
        ...
        footer.innerHTML = `
            <div class="sidebar-user-card" onclick="navigate('profile'); closeMenu();">
                ...
                <div class="sidebar-user-info-block">
                    ...
                    <div class="sidebar-user-xp"><div class="sidebar-user-xp-fill" style="width:${xpProgress}%"></div></div>
                </div>
            </div>
            <button class="sidebar-logout-btn" onclick="event.stopPropagation(); logout();"><span class="logout-icon">🚪</span><span>Выйти</span></button>
        `;

        const activeLink = nav.querySelector(`a[data-page="${currentPage}"]`);
        if (activeLink) activeLink.classList.add('active');
    } else {
        nav.innerHTML = `<a class="active" data-page="home" onclick="navigate('home'); closeMenu();"><span class="icon">🏠</span><span>Главная</span></a>`;
        footer.innerHTML = `<button class="sidebar-login-btn" onclick="showLoginModal(); closeMenu();"><span>🚀</span><span>Войти в аккаунт</span></button>`;
    }
}
...
function renderSkeletons(count = 12) {
    ...
    for (let i = 0; i < count; i++) {
        html += `<div class="skeleton-card"><div class="skeleton-poster"></div><div class="skeleton-title"></div><div class="skeleton-info"></div></div>`;
    }
    return html;
}
...
async function loadCatalog(targetPage = null) {
    ...
    if (!allItems.length) grid.innerHTML = renderSkeletons(CATALOG_LIMIT);
    ...
    try {
        ...
        if (result && result.items && result.items.length > 0) {
            ...
            if (!searchValue && !genre && page === 1) {
                try {
                    sessionStorage.setItem('onika_catalog_cache', JSON.stringify({
                        items: allItems.slice(0, CATALOG_LIMIT), time: Date.now()
                    }));
                } catch(e) {}
            }
            ...
            if (stats) {
                ...
                stats.textContent = searchText ? `${searchText} — стр. ${page}` : `📄 Страница ${page}`;
            }
...
function renderPagination(currentPageNum, itemsCount) {
    ...
    if (hasNext) {
        ...
        if (nextPageNum > endPage + 1) pagesHtml += `<span class="pagination-dots">...</span>`;
...
function goToPrevPage() { if (page > 1) goToPage(page - 1); }
function goToNextPage() { if (!isAllLoaded) goToPage(page + 1); }
...
function renderCatalog(list) {
    ...
    if (!list || list.length === 0) {
        grid.innerHTML = `<div style="text-align:center;padding:60px 20px;color:var(--text-muted);grid-column:1/-1;"><div style="font-size:64px;margin-bottom:16px;">🔍</div><p style="font-size:18px;font-weight:600;">Ничего не найдено</p></div>`;
        return;
    }
...
async function randomAnime() {
    ...
    resultContainer.innerHTML = `<div class="random-loading"><div class="random-spinner-new"></div><span class="random-loading-text">🌀 Подбираем идеальное аниме...</span></div>`;

    try {
        const items = await API.getRandom(1);
        if (!items || !items.length) {
            resultContainer.innerHTML = `<div class="random-error-new"><span class="error-icon">😅</span><p>Не удалось найти аниме. Попробуйте ещё раз!</p></div>`;
        ...
        resultContainer.innerHTML = `<div class="random-error-new"><span class="error-icon">⚠️</span><p>Ошибка загрузки. Попробуйте позже.</p></div>`;
        if (triggerBtn) triggerBtn.classList.remove('loading');
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
        if (!e.target.closest('.catalog-search-wrapper')) autocompleteContainer.style.display = 'none';
    });
});
...
async function openDetail(id) {
    ...
    if (!id) { showToast('Ошибка ID', 'error'); return; }
...
function showDetail(anime) {
    ...
    try {
        ...
        if (metaEl) {
            ...
            let metaHtml = '';
            if (score > 0) metaHtml += `<span class="detail-score ${scoreClass}"><span class="star-icon">⭐</span><span>${score.toFixed(1)}</span></span>`;
            if (year && year !== '--') metaHtml += `<span class="detail-meta-item">📅 ${year}</span>`;
            if (episodes && episodes !== '?') metaHtml += `<span class="detail-meta-item">📺 ${episodes} эп.</span>`;
            if (anime.status) metaHtml += `<span class="detail-meta-item">${anime.status}</span>`;
        ...
        if (favBtn) {
            ...
            favBtn.onclick = function() {
                ...
                setTimeout(() => {
                    ...
                    if (isFavNow) { try { spawnFavConfetti(favBtn); } catch(e) {} }
                }, 100);
            };
        }
...
function updateFavButton(btn, isFav) {
    ...
    if (isFav) {
        ...
        btn.innerHTML = `<svg width="20" height="20" viewBox="0 0 24 24" fill="#fff"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>В избранном`;
    } else {
        btn.classList.remove('active');
        btn.innerHTML = `<svg width="20" height="20"><use href="icons/icons.svg#icon-heart-empty"/></svg>В избранное`;
    }
}
...
function spawnFavConfetti(btn) {
    ...
    try {
        ...
        for (let i = 0; i < 24; i++) {
            ...
            html += `<div style="position:absolute;left:${x}px;top:${y}px;width:${size}px;height:${size}px;background:${color};border-radius:${borderRadius};opacity:0;animation: favConfettiFly ${duration}s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards;animation-delay:${delay}s;--tx:${tx}px;--ty:${ty}px;--rot:${rotate}deg;"></div>`;
        }

        const style = document.createElement('style');
        style.textContent = `@keyframes favConfettiFly { 0% { opacity: 1; transform: translate(-50%, -50%) scale(0.3) rotate(0deg); } 50% { opacity: 1; } 100% { opacity: 0; transform: translate(calc(-50% + var(--tx)), calc(-50% + var(--ty))) scale(0.6) rotate(var(--rot)); } }`;
...
function extractDominantColor(imgUrl, callback) {
    if (!imgUrl) { callback('rgba(108, 92, 231, 0.3)'); return; }
...
function renderWatchProgress(animeTitle, anime) {
    ...
    const user = DB.get('currentUser');
    if (!user || !animeTitle) { container.style.display = 'none'; return; }
...
function openKodiModal() {
    ...
    if (!modal) {
        ...
        modal.innerHTML = `<div class="modal-box" style="max-width:520px;"><button class="modal-close" onclick="document.getElementById('kodiModal').style.display='none'"><svg width="24" height="24"><use href="icons/icons.svg#icon-close"/></svg></button><div class="modal-icon">🎞️</div><h2>Kodi плеер</h2><p>Смотрите аниме через бесплатное приложение Kodi с аддоном для аниме.</p><div class="kodi-instruction"><h3>📥 Как установить:</h3><ol><li>Скачайте Kodi с <a href="https://kodi.tv/download" target="_blank" rel="noopener">kodi.tv</a></li><li>Установите приложение на устройство</li><li>Откройте Kodi → Дополнения → Установить из репозитория</li><li>Найдите аддон для аниме (например, AniList)</li><li>Наслаждайтесь просмотром!</li></ol></div><div class="btn-row"><a href="https://kodi.tv/download" target="_blank" rel="noopener" class="btn-primary" style="text-decoration:none;text-align:center;display:block;flex:1;padding:12px;">📥 Скачать Kodi</a><button class="btn-cancel" onclick="document.getElementById('kodiModal').style.display='none'">Закрыть</button></div></div>`;
...
async function watchOnVK() {
    ...
    container.style.display = 'block';
    container.innerHTML = `<div class="watch-embed-wrapper"><div class="player-loader"><div class="player-loader-ring"></div><div class="player-loader-text">Ищем в базе VK...</div></div></div>`;
    try {
...
async function watchOnDeep() {
    ...
    const container = document.getElementById('watchEmbedContainer');
    container.style.display = 'block';
    container.innerHTML = `<div class="watch-embed-wrapper"><div class="player-loader"><div class="player-loader-ring"></div><div class="player-loader-text">Ищем в базе Deep...</div></div></div>`;
    try {
        const response = await fetch('/api/sources/' + encodeURIComponent(title));
        const data = await response.json();
...
function renderSourceEpisodes(source, episodes, title) {
    ...
    episodes.forEach((ep, index) => {
        episodesListHtml += `<button onclick="playSourceEpisode('${source}', ${index}, '${title.replace(/'/g, "\\'")}')" class="source-episode-btn" data-index="${index}" style="padding:8px 14px;border-radius:10px;border:1px solid ${index === 0 ? 'var(--neon-cyan)' : 'rgba(0,245,255,0.1)'};background:${index === 0 ? 'rgba(0,245,255,0.1)' : 'rgba(0,245,255,0.02)'};color:${index === 0 ? 'var(--neon-cyan)' : 'var(--text-primary)'};cursor:pointer;font-size:13px;font-weight:600;transition:all 0.3s ease;margin:3px;">${sourceIcons[source]} Серия ${ep.episode}</button>`;
    });

    container.innerHTML = `
        <div class="watch-embed-header"><h4><span>${sourceIcons[source]}</span> ${sourceNames[source]} — ${title}</h4><button class="watch-embed-close" onclick="closeWatchEmbed()">✕ Закрыть</button></div>
        <div style="padding:16px 20px;"><div style="font-size:12px;color:var(--text-muted);margin-bottom:10px;">📺 Доступно серий: ${episodes.length}</div><div style="display:flex;flex-wrap:wrap;gap:4px;max-height:200px;overflow-y:auto;">${episodesListHtml}</div></div>
        <div id="sourcePlayerArea" class="watch-embed-wrapper"><div class="player-loader"><div class="player-loader-ring"></div><div class="player-loader-text">Нажмите на серию</div></div></div>
        <div style="margin-top:12px;text-align:center;padding:0 16px 16px;"><a href="${firstEp.url}" target="_blank" rel="noopener" style="color:var(--neon-cyan);font-size:13px;text-decoration:none;">🔗 Открыть в новой вкладке</a></div>
    `;
    ...
    try {
        ...
        if (canEmbed) {
            ...
            if (user) {
                ...
                try { renderWatchProgress(title, { episodes: data.sources[source].length }); } catch(e) {}
            }
        } else {
            playerArea.innerHTML = `<div style="position:absolute;top:0;left:0;width:100%;height:100%;display:flex;align-items:center;justify-content:center;color:#aaa;flex-direction:column;gap:16px;background:rgba(0,0,0,0.85);"><span style="font-size:56px;">🎬</span><span style="font-size:16px;font-weight:600;color:#fff;">Серия ${episode.episode}</span><span style="font-size:13px;color:#888;">Откройте источник в новой вкладке</span><a href="${episode.url}" target="_blank" rel="noopener" style="padding:12px 32px;border-radius:50px;background:linear-gradient(135deg,var(--neon-cyan),var(--neon-purple));color:#fff;text-decoration:none;font-weight:700;font-size:14px;box-shadow:0 4px 20px rgba(0,245,255,0.2);">▶️ Открыть серию ${episode.episode}</a></div>`;
        }
...
function renderNoSources(source, title) {
    ...
    const sourceUrls = { vk: `https://vk.com/videos-201142575?q=${encodeURIComponent(title)}`, deep: `https://deep-ent.ru/search?q=${encodeURIComponent(title)}` };

    container.innerHTML = `
        <div class="watch-embed-header"><h4>🔍 ${sourceNames[source]} — ${title}</h4><button class="watch-embed-close" onclick="closeWatchEmbed()">✕ Закрыть</button></div>
        <div style="padding:24px;text-align:center;">
            <div style="padding:16px;background:rgba(255,215,0,0.05);border-radius:14px;border:1px solid rgba(255,215,0,0.2);margin-bottom:16px;"><strong style="color:#ffd700;">⚠️ Пока нет ссылок в базе</strong><br><span style="font-size:13px;color:var(--text-secondary);">Мы ищем видео вручную. Попробуйте найти через поиск источника.</span></div>
            <div style="display:flex;flex-direction:column;gap:10px;max-width:400px;margin:0 auto;">
                <a href="${sourceUrls[source]}" target="_blank" rel="noopener" style="padding:14px 20px;border-radius:14px;background:rgba(0,245,255,0.08);border:1px solid rgba(0,245,255,0.2);color:var(--neon-cyan);text-decoration:none;font-weight:700;font-size:13px;">🔍 Найти «${title}» вручную</a>
                <a href="https://vk.com/deep" target="_blank" rel="noopener" style="padding:14px 20px;border-radius:14px;background:rgba(108,92,231,0.08);border:1px solid rgba(108,92,231,0.2);color:var(--accent-glow);text-decoration:none;font-weight:700;font-size:13px;">🌟 Сообщество DEEP</a>
            </div>
        </div>
    `;
}
    ...
    fetch('/api/comments/' + encodeURIComponent(animeName))
        ...
        .then(comments => {
            if (!comments || !comments.length) {
                container.innerHTML = `<div style="color:#666;text-align:center;padding:40px 20px;"><div style="font-size:48px;margin-bottom:10px;">💬</div><div>Нет комментариев</div><div style="font-size:13px;color:var(--text-muted);margin-top:6px;">Будь первым!</div></div>`;
                return;
            }
            const user = DB.get('currentUser');
            let html = '';
        ...
        .catch(() => { container.innerHTML = '<div style="color:#666;text-align:center;padding:20px;">⚠️ Ошибка загрузки</div>'; });
}

function openUserFromComment(userName) {
    const user = DB.get('currentUser');
    if (!user) { showToast('Войдите в аккаунт!', 'error'); return; }
    if (user.name === userName) navigate('profile');
...
function addComment() {
    ...
    .catch(() => { if (btn) { btn.style.pointerEvents = ''; btn.style.opacity = ''; } showToast('Ошибка сети', 'error'); });
}
...
function renderFavorites() {
    ...
    if (!user) {
        container.innerHTML = `<div class="fav-grid"><div class="fav-empty"><div class="fav-empty-icon">🔐</div><h3 class="fav-empty-title">Войдите в аккаунт</h3><p class="fav-empty-desc">Чтобы сохранять любимые аниме в избранное</p><button class="fav-empty-btn" onclick="showLoginModal()">🚀 Войти</button></div></div>`;
        updateFavStats([]);
        return;
    }
    ...
    const favs = DB.getUserData(user.name, 'favorites', []);
    if (favs.length === 0) {
        container.innerHTML = `<div class="fav-grid"><div class="fav-empty"><div class="fav-empty-icon">💔</div><h3 class="fav-empty-title">Пока пусто</h3><p class="fav-empty-desc">Добавляй аниме в избранное, нажимая на сердечко</p><button class="fav-empty-btn" onclick="navigate('home')">🎬 Найти аниме</button></div></div>`;
        updateFavStats([]);
        return;
    }
    updateFavStats(favs);
...
function applyFavFilters() {
    ...
    if (searchValue) favs = favs.filter(name => name.toLowerCase().includes(searchValue));
    ...
    sectionConfig.forEach(cfg => {
        ...
        html += `<div class="fav-section"><div class="fav-section-header"><span class="fav-section-icon">${cfg.icon}</span><h3 class="fav-section-title">${cfg.title}</h3><span class="fav-section-count">${sectionItems.length}</span></div><div class="fav-grid${isListView ? ' list-view' : ''}">${sectionItems.map((name, i) => renderFavCard(name, i)).join('')}</div></div>`;
    });

    if (!html) html = `<div class="fav-grid"><div class="fav-empty"><div class="fav-empty-icon">🔍</div><h3 class="fav-empty-title">Ничего не найдено</h3><p class="fav-empty-desc">Попробуйте изменить параметры поиска</p></div></div>`;
    container.innerHTML = html;
    updateMassCount();
}
...
function sortFavorites(favs, sortValue) {
    ...
    return [...favs].sort((a, b) => {
        ...
        let valA, valB;
        switch (field) {
            case 'title': valA = a.toLowerCase(); valB = b.toLowerCase(); break;
            case 'score': valA = itemA?.score || 0; valB = itemB?.score || 0; break;
            case 'episodes': valA = parseInt(itemA?.episodes) || 0; valB = parseInt(itemB?.episodes) || 0; break;
            case 'added':
            default:
...
function groupFavoritesByStatus(favs) {
    const groups = { watching: [], planned: [], completed: [], other: [] };
    favs.forEach(name => {
        const item = findAnimeByName(name);
        const status = item?.status || '';
        if (status.includes('Онгоинг') || status.includes('ongoing')) groups.watching.push(name);
        else if (status.includes('Завершено') || status.includes('released')) groups.completed.push(name);
        else if (status.includes('Анонс') || status.includes('anons')) groups.planned.push(name);
...
function findAnimeByName(name) {
    for (const id in allData) {
        ...
        if (item && item.title === name) return item;
    }
    return null;
}
...
function openDetailFromFav(name) {
    if (!name) return;
    const item = findAnimeByName(name);
    if (item) openDetail(item.mal_id || item.id);
    else searchAndOpen(name);
}
...
function updateFavStats(favs) {
    ...
    favs.forEach(name => {
        ...
        if (item) {
            ...
            totalEpisodes += eps;
            if (item.score > 0) { totalScore += item.score; scoreCount++; }
        }
    });
    ...
    document.getElementById('favStatScore').textContent = scoreCount > 0 ? (totalScore / scoreCount).toFixed(1) : '—';
}

function filterFavorites() {
    const searchInput = document.getElementById('favSearchInput');
    const clearBtn = document.getElementById('favSearchClear');
    if (searchInput && clearBtn) clearBtn.style.display = searchInput.value.length > 0 ? 'flex' : 'none';
    applyFavFilters();
}
...
function toggleFavView() {
    ...
    btn.textContent = btn.classList.contains('active') ? '🔲 Сетка' : '📋 Список';
    applyFavFilters();
}
...
function randomFromFav() {
    ...
    if (!user) { showToast('Войдите в аккаунт!', 'error'); return; }
    const favs = DB.getUserData(user.name, 'favorites', []);
    if (favs.length === 0) { showToast('Избранное пусто!', 'warning'); return; }
    const randomName = favs[Math.floor(Math.random() * favs.length)];
    const item = findAnimeByName(randomName);
    if (item) { showToast(`🎲 Выбрано: ${randomName}`, 'success'); openDetail(item.mal_id || item.id); }
    else searchAndOpen(randomName);
}
...
function deleteSelectedFav() {
    ...
    if (selected.length === 0) { showToast('Ничего не выбрано', 'warning'); return; }
    ...
    showConfirmModal('🗑 Удалить выбранные?', `Удалить ${selected.length} аниме из избранного?`, function() {
        ...
        setTimeout(() => {
            ...
            updateMassCount();
            const remaining = document.querySelectorAll('.fav-card').length;
            if (remaining === 0) { renderFavorites(); toggleMassMode(); }
        }, 600);
...
function getUserMetrics(user) {
    ...
    let commentsCount = 0;
    if (window._myCommentsCache && window._myCommentsCache.user === user) commentsCount = window._myCommentsCache.count;
    const watching = DB.getUserData(user, 'continueWatching', {});
    let episodesWatched = 0;
    for (const anime in watching) episodesWatched += watching[anime].episode || 0;
    return { favorites: favs.length, comments: commentsCount, episodes: episodesWatched };
}
...
async function renderAchievements() {
    ...
    if (!user) {
        grid.innerHTML = `<div class="ach-empty-v2"><span class="ach-empty-icon-v2">🔐</span><h3 class="ach-empty-title-v2">Войдите в аккаунт</h3><p class="ach-empty-desc-v2">Чтобы видеть свои достижения и прогресс</p><button class="ach-empty-btn-v2" onclick="showLoginModal()">🚀 Войти</button></div>`;
    ...
    } catch(e) { window._myCommentsCache = { user: user.name, count: 0 }; }
    ...
    if (allAchievements.length === 0) {
        grid.innerHTML = `<div class="ach-empty-v2"><span class="ach-empty-icon-v2">🔍</span><h3 class="ach-empty-title-v2">Пока нет достижений</h3></div>`;
        return;
    }

    let html = '';
    allAchievements.forEach((ach, index) => { html += renderAchCardV2(ach, index, activeTitle); });
    grid.innerHTML = html;
}
...
function sortAchievements(list, sortValue) {
    ...
    return [...list].sort((a, b) => {
        let valA, valB;
        switch (field) {
            case 'rarity': valA = RARITY_ORDER[a.rarity] || 0; valB = RARITY_ORDER[b.rarity] || 0; break;
            case 'earned': valA = a.isEarned ? 1 : 0; valB = b.isEarned ? 1 : 0; break;
            case 'locked': valA = a.isEarned ? 0 : 1; valB = b.isEarned ? 0 : 1; break;
...
function renderAchCardV2(ach, index, activeTitle) {
    ...
    const progressText = ach.isEarned ? `✅ Получено` : `${ach.current} / ${ach.target}`;
    const footerHtml = ach.isEarned
        ? `<div class="ach-title-info">🎖️ <strong>${ach.title}</strong></div><button class="ach-btn-v2 ${isActive ? 'active' : 'set'}" onclick="event.stopPropagation(); setActiveTitle('${ach.id}')">${isActive ? '👑 Активен' : '⭐ Установить'}</button>`
        : `<div class="ach-title-info">🔒 <span>Закрыто</span></div><button class="ach-btn-v2 locked" disabled>🔒 ${ach.progress}%</button>`;

    return `<div class="ach-card-v2 ${ach.isEarned ? 'earned' : 'locked'} rarity-${ach.rarity}" data-index="${index}" onclick="openAchModal('${ach.id}')" style="animation-delay: ${index * 0.05}s;"><div class="ach-shine"></div>${ach.isEarned ? `<div class="ach-earned-check">✓</div>` : ''}<div class="ach-card-header"><div class="ach-icon-wrap"><span class="ach-icon-v2">${ach.icon}</span></div><div class="ach-rarity-badge ${ach.rarity}">${rarityLabel}</div></div><div class="ach-card-body"><h3 class="ach-card-name">${ach.name}</h3><p class="ach-card-desc">${ach.desc}</p><div class="ach-progress-v2"><div class="ach-progress-header"><span class="ach-progress-label">Прогресс</span><span class="ach-progress-value">${progressText}</span></div><div class="ach-progress-bar-v2"><div class="ach-progress-fill-v2" style="width: ${ach.progress}%;"></div></div></div></div><div class="ach-card-footer">${footerHtml}</div></div>`;
}
...
function renderActiveTitle(activeTitle) {
    ...
    if (!container) return;
    if (!activeTitle) { container.innerHTML = `<div class="ach-no-title">🎖️ Титул не установлен</div>`; return; }
    const ach = ACHIEVEMENTS_LIST.find(a => a.id === activeTitle);
    if (!ach) { container.innerHTML = `<div class="ach-no-title">🎖️ Титул не установлен</div>`; return; }
    container.innerHTML = `<div class="ach-active-title"><span class="ach-active-title-icon">🎖️</span><span>Титул: ${ach.title}</span></div>`;
}
...
function setActiveTitle(achId) {
    ...
    if (currentTitle === achId) { DB.setActiveTitle(user.name, null); showToast('👑 Титул снят', 'info'); }
    else { DB.setActiveTitle(user.name, achId); const ach = ACHIEVEMENTS_LIST.find(a => a.id === achId); showToast(`👑 Титул "${ach?.title}" установлен!`, 'success'); spawnTitleConfetti(); }
    renderAchievements();
...
function openAchModal(achId) {
    ...
    modal.innerHTML = `<div class="modal-box" style="max-width:460px;"><button class="modal-close" onclick="closeModal('achDetailModal')"><svg width="24" height="24"><use href="icons/icons.svg#icon-close"/></svg></button><div class="ach-modal-content"><div class="ach-modal-icon ${ach.rarity}">${ach.icon}</div><div class="ach-modal-rarity ${ach.rarity}">${rarityLabel}</div><h2 class="ach-modal-name">${ach.name}</h2><p class="ach-modal-desc">${ach.desc}</p><div class="ach-modal-progress"><div class="ach-modal-progress-bar"><div class="ach-modal-progress-fill" style="width: ${progress}%;"></div></div><div class="ach-modal-progress-text">Прогресс: <strong>${current} / ${ach.target}</strong> (${progress}%)</div></div>${isEarned ? `<button class="btn-primary" onclick="setActiveTitle('${achId}'); closeModal('achDetailModal');" style="width:100%; padding:14px; font-size:15px;">${isActive ? '👑 Снять титул' : '⭐ Установить титул'}</button>` : `<div style="padding:12px; background:rgba(255,255,255,0.03); border-radius:12px; font-size:13px; color:var(--text-muted);">🔒 ${progress > 0 ? 'Продолжай в том же духе!' : 'Начни путь к этому достижению'}</div>`}</div></div>`;
    modal.style.display = 'flex';
}
...
function showAchUnlock(ach) {
    ...
    if (iconEl) { iconEl.textContent = ach.icon; iconEl.className = 'ach-unlock-icon' + (ach.rarity === 'legendary' ? ' legendary' : ''); }
    if (nameEl) nameEl.textContent = ach.name;
    if (descEl) descEl.textContent = ach.desc;
    overlay.classList.add('show');
    if (ach.rarity === 'legendary' || ach.rarity === 'epic') spawnBigConfetti(ach.rarity);
}
...
function spawnBigConfetti(rarity) {
    const colors = rarity === 'legendary' ? ['#ffd700', '#ffed4e', '#f39c12', '#ff2d78', '#b44aff'] : ['#b44aff', '#a29bfe', '#6c5ce7', '#fd79a8'];
    let html = '';
    for (let i = 0; i < 80; i++) {
        const x = Math.random() * 100;
...
function renderProfile() {
    ...
    if (!avatarFound) {
        ...
        if (letter) { letter.style.display = 'flex'; letter.textContent = user.name[0].toUpperCase(); }
    }
    ...
    if (titleBadge && activeTitle) {
        const ach = ACHIEVEMENTS_LIST.find(a => a.id === activeTitle);
        if (ach) { titleBadge.textContent = '🎖️ ' + ach.title; titleBadge.style.display = 'inline-flex'; }
        else titleBadge.style.display = 'none';
    } else if (titleBadge) titleBadge.style.display = 'none';
...
function animateNumber(elementId, targetValue, duration = 1200) {
    ...
    function update(currentTime) {
        ...
        else { el.textContent = targetValue; el._animFrame = null; }
    }
...
function animateTimeCounter(elementId, totalSeconds, duration = 1200) {
    ...
    function update(currentTime) {
        ...
        if (progress < 1) el._animFrame = requestAnimationFrame(update);
        else { el.textContent = formatTime(totalSeconds); el._animFrame = null; }
    }
    el._animFrame = requestAnimationFrame(update);
}
...
function getCommentCount(user) {
    ...
    for (const key in allComments) {
        allComments[key].forEach(function(c) { if (c.user === user) count++; });
    }
    return count;
}
...
function getPosterForAnime(name) {
    for (const id in allData) {
        if (allData[id] && allData[id].title === name) return allData[id].images?.jpg?.image_url || '';
    }
    return '';
}

function changeBanner() {
    const colors = ['linear-gradient(135deg, #1a1a3e, #2d1b69, #6c5ce7)', 'linear-gradient(135deg, #0c0c1e, #1a0a2e, #4a2b7a)', 'linear-gradient(135deg, #1a0a0a, #3d1a1a, #7a2b2b)', 'linear-gradient(135deg, #0a1a0a, #1a3d1a, #2b7a4a)', 'linear-gradient(135deg, #2d1b69, #6c5ce7, #fd79a8)', 'linear-gradient(135deg, #001a33, #004d7a, #00f5ff)', 'linear-gradient(135deg, #3d1a4d, #7a2b7a, #ff2d78)'];
...
function renderProfileAchievements(user) {
    ...
    if (recent.length === 0) { grid.innerHTML = '<div style="color:var(--text-muted);text-align:center;padding:12px;">Нет достижений</div>'; return; }
    let html = '';
    recent.forEach((id, index) => {
        const ach = ACHIEVEMENTS_LIST.find(a => a.id === id);
        if (ach) html += `<div class="profile-ach-item" style="animation-delay: ${index * 0.1}s;"><span class="ach-icon">${ach.icon}</span><div class="ach-name">${ach.name}</div>${ach.title ? `<div class="ach-title">🎖️ ${ach.title}</div>` : ''}</div>`;
    });
    grid.innerHTML = html;
}
...
function renderTopUsers() {
    ...
    for (const u in users) {
        ...
        data[u] = { name: u, favs: favs.length, comments, achievements: earned.length, onlineTime, xp, title: titleName, isOnline: (Date.now() - lastSeen) < 300000 };
    }
    const sorted = Object.values(data).sort((a, b) => b.xp - a.xp).slice(0, 20);
    if (sorted.length === 0) { container.innerHTML = '<div style="color:var(--text-muted);text-align:center;padding:30px;"><span style="font-size:48px;display:block;">👑</span><p>Нет пользователей</p></div>'; return; }

    const medals = ['🥇', '🥈', '🥉', '4️⃣', '5️⃣', '6️⃣', '7️⃣', '8️⃣', '9️⃣', '🔟'];
    const avatarGradients = ['avatar-gradient-1', 'avatar-gradient-2', 'avatar-gradient-3', 'avatar-gradient-4', 'avatar-gradient-5', 'avatar-gradient-6', 'avatar-gradient-7', 'avatar-gradient-8', 'avatar-gradient-9', 'avatar-gradient-10'];

    let html = `<div class="top-users-wrapper"><div class="top-users-header"><h3>👑 Топ пользователей</h3><span class="top-update-time">🔄 ${new Date().toLocaleTimeString()}</span></div><div style="overflow-x:auto;"><table class="top-users-table"><thead><tr><th>#</th><th>Пользователь</th><th class="hide-mobile">📚</th><th class="hide-mobile">💬</th><th>🏆</th><th>⏱</th><th>⭐ XP</th></tr></thead><tbody>`;
    ...
    sorted.forEach(function(user, index) {
        ...
        html += `<tr class="${rankClass}"><td class="rank-cell">${medal}</td><td><div class="user-info-cell"><div class="user-avatar-mini ${avatarGrad}">${user.name[0].toUpperCase()}</div><div><div class="user-name-cell">${user.name} <span style="font-size:11px;color:${user.isOnline ? '#2ecc71' : '#666'};">${user.isOnline ? '🟢' : '🟡'}</span></div>${user.title ? `<div class="user-title-cell">🎖️ ${user.title}</div>` : ''}</div></div></td><td class="stat-cell hide-mobile"><span class="stat-number">${user.favs}</span></td><td class="stat-cell hide-mobile"><span class="stat-number">${user.comments}</span></td><td class="stat-cell"><span class="stat-number">${user.achievements}</span></td><td class="time-cell"><div class="time-value">${formatTime(user.onlineTime)}</div></td><td><div class="xp-bar-wrapper"><div class="xp-bar-bg"><div class="xp-bar-fill" style="width:${xpPercent}%;"></div></div><div class="xp-text">${user.xp} XP</div></div></td></tr>`;
    });
...
function showToast(message, type) {
    ...
    toast.style.cssText = `position: fixed; bottom: 90px; left: 50%; transform: translateX(-50%); background: ${colors[type] || colors.info}; color: #fff; padding: 14px 28px; border-radius: 14px; font-weight: 600; z-index: 99999; max-width: 90%; text-align: center; border: 1px solid rgba(108,92,231,0.2); backdrop-filter: blur(20px); font-size: 14px; animation: fadeInUp 0.4s ease forwards; box-shadow: 0 10px 40px rgba(0,0,0,0.5);`;
...
function editProfile(type) {
    ...
    if (type === 'name') { title.textContent = '✏️ Изменить никнейм'; input.value = user.name; input.type = 'text'; }
    else if (type === 'email') { title.textContent = '✏️ Изменить email'; input.value = user.email; input.type = 'email'; }
    else if (type === 'pass') { title.textContent = '🔑 Изменить пароль'; input.value = ''; input.type = 'password'; }
    else if (type === 'bio') {
...
function saveEdit() {
    ...
    } else showToast('❌ Изменение этого поля пока не поддерживается', 'warning');
}
...
function renderMyComments() {
    ...
    if (!user) {
        container.innerHTML = `<div class="mc-list"><div class="mc-empty"><div class="mc-empty-icon">🔐</div><h3 class="mc-empty-title">Войдите в аккаунт</h3><p class="mc-empty-desc">Чтобы видеть свои комментарии</p><button class="mc-empty-btn" onclick="showLoginModal()">🚀 Войти</button></div></div>`;
        updateMcStats([]);
        return;
    }

    container.innerHTML = `<div style="text-align:center;padding:40px;color:#888;"><div class="spinner-small"></div><br>⏳ Загрузка комментариев...</div>`;
    ...
    fetch('/api/comments/all')
        ...
        .catch(err => {
            container.innerHTML = `<div class="mc-list"><div class="mc-empty"><div class="mc-empty-icon">⚠️</div><h3 class="mc-empty-title">Ошибка загрузки</h3><p class="mc-empty-desc">${err.message}</p><button class="mc-empty-btn" onclick="renderMyComments()">🔄 Попробовать снова</button></div></div>`;
        });
}
...
function applyMcFilters() {
    ...
    if (searchValue) comments = comments.filter(c => (c.text || '').toLowerCase().includes(searchValue) || (c.anime || '').toLowerCase().includes(searchValue));
    ...
    if (comments.length === 0) {
        container.innerHTML = `<div class="mc-list"><div class="mc-empty"><div class="mc-empty-icon">💬</div><h3 class="mc-empty-title">${searchValue ? 'Ничего не найдено' : 'Пока нет комментариев'}</h3><p class="mc-empty-desc">${searchValue ? 'Попробуйте изменить поиск' : 'Оставляй комментарии к аниме, и они появятся здесь'}</p>${!searchValue ? '<button class="mc-empty-btn" onclick="navigate(\'home\')">🎬 Найти аниме</button>' : ''}</div></div>`;
        return;
    }
    if (mcGroupMode) renderMcGroupedByDate(comments);
    else renderMcList(comments);
    updateMcMassCount();
}
...
function sortMyComments(comments, sortValue) {
    ...
    return [...comments].sort((a, b) => {
        let valA, valB;
        switch (field) {
            case 'date': valA = a.created_at ? new Date(a.created_at).getTime() : 0; valB = b.created_at ? new Date(b.created_at).getTime() : 0; break;
            case 'length': valA = a.text?.length || 0; valB = b.text?.length || 0; break;
            case 'anime': valA = (a.anime || '').toLowerCase(); valB = (b.anime || '').toLowerCase(); break;
            default: valA = 0; valB = 0;
        }
        if (valA < valB) return -1 * dir;
        if (valA > valB) return 1 * dir;
...
function renderMcList(comments) {
    ...
    comments.forEach((c, index) => { html += renderMcItem(c, index, maxLength); });
    html += '</div>';
    container.innerHTML = html;
}
...
function renderMcGroupedByDate(comments) {
    ...
    groupConfig.forEach(cfg => {
        ...
        html += `<div class="mc-section"><div class="mc-section-header"><span class="mc-section-icon">${cfg.icon}</span><h3 class="mc-section-title">${cfg.title}</h3><span class="mc-section-count">${items.length}</span></div><div class="mc-list">${items.map((c, i) => renderMcItem(c, i, maxLength)).join('')}</div></div>`;
    });
    container.innerHTML = html;
}
...
function onMcItemClick(id) {
    if (mcMassMode) { toggleMcSelection(id); return; }
    const comment = myCommentsAll.find(c => c.id === id);
    if (comment) searchAndOpen(comment.anime);
}

function toggleMcSelection(id) {
    if (mcSelectedIds.has(id)) mcSelectedIds.delete(id);
    else mcSelectedIds.add(id);
    const item = document.querySelector(`.mc-item[data-id="${id}"]`);
    if (item) item.classList.toggle('selected', mcSelectedIds.has(id));
    updateMcMassCount();
}
...
function selectAllMc() { myCommentsFiltered.forEach(c => mcSelectedIds.add(c.id)); applyMcFilters(); }
function deselectAllMc() { mcSelectedIds.clear(); applyMcFilters(); }

function deleteSelectedMc() {
    if (mcSelectedIds.size === 0) { showToast('Ничего не выбрано', 'warning'); return; }
    showConfirmModal('🗑 Удалить выбранные?', `Удалить ${mcSelectedIds.size} комментариев?`, function() {
        const user = DB.get('currentUser');
        if (!user) return;
        const ids = Array.from(mcSelectedIds);
        let deleted = 0;
        const promises = ids.map(id => fetch('/api/comments/' + id, {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ user_name: user.name })
        }).then(res => res.json()).then(data => { if (data.success) deleted++; }).catch(() => {}));
        Promise.all(promises).then(() => {
            ...
            if (btn) { btn.classList.remove('active'); btn.innerHTML = '☑️ Выбрать'; }
            renderMyComments();
        });
    });
}
...
function deleteMcComment(id) {
    showConfirmModal('🗑 Удалить комментарий', 'Вы уверены?', function() {
        ...
        fetch('/api/comments/' + id, {
            ...
            if (data.success) { showToast('🗑 Комментарий удалён', 'success'); renderMyComments(); }
            else showToast(data.error || 'Ошибка', 'error');
...
function toggleMcGroup() {
    ...
    if (btn) btn.classList.toggle('active', mcGroupMode);
    applyMcFilters();
}
